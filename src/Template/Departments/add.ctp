...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link(__('Manager Home'), ['controller'=>'Manager','action' => 'home']) ?></li>
        <li><?= $this->Html->link(__('List Departments'), ['action' => 'index']) ?></li>
      
    </ul>
</nav>
<div class="departments form large-9 medium-8 columns content">
    <?= $this->Form->create($department,['autocomplete'=>'off']) ?>
    <fieldset>
        <legend><?= __('Add Department') ?></legend>
        <?php
            echo $this->Form->control('name',['label'=>'Department Name:','required'=>'true']);
        ?>
    </fieldset>
