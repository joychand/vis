<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\PowerInfra $powerInfra
 */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('Edit Power Infra'), ['action' => 'edit', $powerInfra->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Power Infra'), ['action' => 'delete', $powerInfra->id], ['confirm' => __('Are you sure you want to delete # {0}?', $powerInfra->id)]) ?> </li>
        <li><?= $this->Html->link(__('List Power Infras'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Power Infra'), ['action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="powerInfras view large-9 medium-8 columns content">
    <h3><?= h($powerInfra->id) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Village Code') ?></th>
            <td><?= h($powerInfra->village_code) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Id') ?></th>
            <td><?= $this->Number->format($powerInfra->id) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Household No') ?></th>
            <td><?= $this->Number->format($powerInfra->household_no) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Electrified Household No') ?></th>
            <td><?= $this->Number->format($powerInfra->electrified_household_no) ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Reference Year') ?></th>
            <td><?= $this->Number->format($powerInfra->reference_year) ?></td>
        </tr>
    </table>
</div>
